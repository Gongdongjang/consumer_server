...
router.get("/storeView", async (req, res, next) => {
  ...
  let message = "에러가 발생했습니다.";
   ...
   try {
       //문제 없으면 try문 실행
        //let resultCode = 200;
        //let message = "storeView";

        const [store_result] = await pool.execute(`SELECT store_id, store_name, store_info, store_hours, store_restDays, store_lat, store_long FROM store`);
