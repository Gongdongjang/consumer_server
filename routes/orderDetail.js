...
router.post("/", async (req, res, next) => {
  ...
  try {
    ...
    const [order_detail] = await pool.execute(
      `SELECT * FROM ggdjang.order join store on ggdjang.order.store_id = store.store_id join payment on payment.md_id = ggdjang.order.md_id join md on ggdjang.order.md_id = md.md_id join md_Img on md_Img.md_id = md.md_id WHERE order.order_cancel = 0 and ggdjang.order.user_id = ?`
    );
